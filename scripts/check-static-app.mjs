...
const requiredFiles = ['index.html', 'src/app/main.ts', 'src/styles.css', 'src/server.ts', 'src/assets/trv-logo.svg'];
...
const js = readFileSync('src/app/main.ts', 'utf8');
for (const text of ['TRV CRM', 'CRM / Leads', 'Propiedades', 'Agenda / Seguimiento', 'localStorage', 'Alertas comerciales', 'Semáforo comercial', 'Cliente caliente sin contactar', 'Ver más', 'Fichas TRV', 'WhatsApp + IA', 'Nuevo cliente', 'trv-logo.svg', 'Guardar ficha TRV', 'Importar automáticamente', 'Pegá el enlace de la propiedad', 'Propiedad sujeta a disponibilidad', 'window.print', 'wa.me']) {
