...
function renderSimple(): void {
  ...
  qs<HTMLElement>('#configuracion').innerHTML = `<div class="settings-grid"><section class="settings-brand-card"><img src="${PRODUCT_BRAND.wordmark}" alt="${PRODUCT_BRAND.name}"><div><span class="eyebrow">Marca del software</span><h3>${PRODUCT_BRAND.name}</h3><p>${PRODUCT_BRAND.tagline}</p><strong>${PRODUCT_BRAND.phrase}</strong><span class="agency-chip">Inmobiliaria configurada: ${AGENCY_BRAND.name}</span></div></section><label>Software<input value="${PRODUCT_BRAND.name}" readonly></label><label>Inmobiliaria<input value="${AGENCY_BRAND.name}" readonly></label><label>WhatsApp público<input value="${AGENCY_BRAND.displayWhatsapp}" readonly></label><label>Marca de las fichas<img src="${AGENCY_BRAND.logo}" alt="${AGENCY_BRAND.name}"></label></div>`;
