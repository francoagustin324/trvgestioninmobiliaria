...
function row(item: Reservation): string { const property=state.crm.properties.find((x)=>x.id===item.propertyId); const offer=item.offerId?state.crm.offers.find((x)=>x.id===item.offerId):undefined; return `<article class="pc-reservation-row" data-reservation-id="${item.id}"><div class="pc-reservation-row-head"><strong>${escapeHtml(label(property,item.propertyId))}</strong><strong>${escapeHtml(item.currency)} ${escapeHtml(money.format(item.amount))}</strong><span class="pc-reservation-status status-${statusClass(item.status)}">${escapeHtml(item.status)}</span></div><div class="pc-reservation-primary"><span>Reserva: ${escapeHtml(item.reservedAt)}${item.expiresAt?` · Vence: ${escapeHtml(item.expiresAt)}`:''}</span></div>${item.offerId?`<p class="pc-reservation-link">Oferta vinculada: #${item.offerId}${offer?` · ${escapeHtml(offer.status)}`:''}</p>`:''}${item.paymentMethod?`<p><b>Pago:</b> ${escapeHtml(item.paymentMethod)}</p>`:''}${item.conditions?`<p><b>Condiciones:</b> ${escapeHtml(item.conditions)}</p>`:''}${item.status==='Activa'?`<form class="pc-reservation-status-form" data-update-reservation="${item.id}"><label>Estado<select name="status" required><option value="">Cambiar estado</option><option value="Cancelada">Cancelada</option><option value="Concretada">Concretada</option></select></label><p class="pc-reservation-error" data-reservation-error role="alert" hidden></p><button type="submit">Guardar estado</button></form>`:''}</article>`; }
