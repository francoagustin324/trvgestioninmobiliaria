...
export function renderMvpUsers(container: HTMLElement): void {
  ...
  container.innerHTML = `<div class="mvp-page-heading"><div><h1>Usuarios</h1><p>Administrá accesos y roles de la inmobiliaria.</p></div>${canManage ? '<button type="button" data-toggle-user-form>Invitar usuario</button>' : ''}</div>${canManage ? `<form id="mvp-user-form" class="mvp-user-form ${state.openForms.member ? '' : 'collapsed'}"><div class="mvp-form-heading"><h2>Invitar usuario</h2><button type="button" class="quiet-button" data-toggle-user-form>Cerrar</button></div><label>Nombre<input name="name" required></label><label>Correo<input name="email" type="email" required></label><label>Rol<select name="role"><option>Corredor</option>${activeMember().role === 'Dueño' ? '<option>Administrador</option>' : ''}</select></label><button type="submit"${sessionReady ? '' : ' disabled'}>${sessionReady ? 'Enviar invitación' : 'Ingresá para invitar'}</button><div data-user-feedback class="auth-message"></div></form>` : ''}<div class="mvp-user-list">${state.crm.teamMembers.map(userRow).join('')}</div>`;
