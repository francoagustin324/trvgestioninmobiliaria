...
test('P1.1-A4 Agenda expone Offer read-only y el workflow conserva autoridad y doble submit bloqueado',()=>{
  const agenda=readFileSync('src/agenda.ts','utf8'), agendaUi=readFileSync('src/agenda-ui.ts','utf8'), workflow=readFileSync('src/offer-workflow.ts','utf8'), ui=readFileSync('src/offer-workflow-ui.ts','utf8'), css=readFileSync('src/offer-workflow.css','utf8'), index=readFileSync('index.html','utf8');
  assert.match(agenda,/nextFollowUp/); assert.match(agenda,/source:\s*'offer'/); assert.match(agenda,/assignmentVisible\(input\.actor\.role, input\.actor\.id, offer\.assignedToId\)/); assert.match(agendaUi,/item\.source === 'visit' \|\| item\.source === 'offer' \|\| item\.source === 'reservation'/); assert.match(agendaUi,/return contextAction\(item\)/); assert.doesNotMatch(`${workflow}\n${ui}`,/state\.crm\.reminders|Reminder|createReminder/i);
