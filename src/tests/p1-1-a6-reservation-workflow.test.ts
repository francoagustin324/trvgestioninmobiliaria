...
test('P1.1-A6 UI/arquitectura: Reserva en Agenda read-only, CTA, estados y ausencia de sistemas paralelos',()=>{
  const workflow=readFileSync('src/reservation-workflow.ts','utf8'),ui=readFileSync('src/reservation-workflow-ui.ts','utf8'),css=readFileSync('src/reservation-workflow.css','utf8'),index=readFileSync('index.html','utf8'),agenda=readFileSync('src/agenda.ts','utf8'),agendaUi=readFileSync('src/agenda-ui.ts','utf8');
  ...
  assert.doesNotMatch(`${workflow}\n${ui}`,/state\.crm\.reminders|createReminder|Commission|Offer\.reservationId/i);assert.match(agenda,/source:\s*'reservation'/);assert.match(agenda,/assignmentVisible\(input\.actor\.role, input\.actor\.id, reservation\.assignedToId\)/);assert.match(agendaUi,/item\.source === 'visit' \|\| item\.source === 'offer' \|\| item\.source === 'reservation'/);assert.match(agendaUi,/return contextAction\(item\)/);assert.doesNotMatch(index,/data-module=["']reservas/i);
});
