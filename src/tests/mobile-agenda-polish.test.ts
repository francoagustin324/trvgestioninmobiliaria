...
test('conserva Completar, Más acciones, apertura, edición y reprogramación', () => {
  assert.ok(ui.includes('data-complete-agenda="client"'));
  assert.ok(ui.includes('data-complete-agenda="reminder"'));
  assert.equal(ui.includes('data-complete-agenda="${item.source}"'), false);
