...
test('SEC-FIX-A1.1 migration es aditiva, V2 explícita y conserva archivos legacy', () => {
  ...
  assert.match(migration, /create function public\.visit_transaction_authority_active_v2\(\s*p_organization_id uuid\s*\)/i);
  assert.match(migration, /create function public\.client_snapshot_cas_v2\(\s*p_organization_id uuid,\s*p_request jsonb,\s*p_force_rollback boolean default false/i);
  assert.match(migration, /create function public\.commercial_visit_mutation_v2\(\s*p_organization_id uuid,\s*p_operation_id uuid,\s*p_operation_type text,\s*p_request jsonb,\s*p_force_rollback boolean default false/i);
...
test('SEC-FIX-A1.1 ejecuta V2 y coexistencia legacy en PostgreSQL 17 efímero', { timeout: 240_000 }, async () => {
  ...
  try {
    ...
    psql(`
      ...
      language sql stable security definer set search_path = '' as $f$
        select exists(
          ...
          where organization_id=target_org and user_id=target_user
            and pg_catalog.lower(status)='active'
        )
      $f$;
    ...
    for (const actor of [noMember, invitedUser, suspendedUser]) {
