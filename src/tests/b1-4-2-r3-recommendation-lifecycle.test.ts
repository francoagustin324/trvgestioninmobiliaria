...
test('R3.23 telemetría no modifica CRM/sync/nextAction/followup/reminder/pipeline', () => {
  ...
  assert.deepEqual(Object.keys(initialData).sort(), ['activityLog', 'clients', 'contacts', 'conversations', 'fichas', 'offers', 'organization', 'properties', 'reservations', 'reminders', 'settings', 'teamMembers', 'visits'].sort());
});
