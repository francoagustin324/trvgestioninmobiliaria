...
function card(client: Client): string {
  ...
  return `<article class="mvp-lead-card mvp-lead-card-with-matches"><div class="mvp-lead-card-main"><div><div class="mvp-lead-name">${tempIcon(client.temperature)}<h3>${escapeHtml(client.name)}</h3><span>${escapeHtml(client.budget || 'Sin presupuesto')}</span></div><p>${client.interest ? `Busca ${escapeHtml(client.interest)}` : 'Sin interés definido'}</p><div class="mvp-lead-contact"><a class="mvp-contact-btn wa" href="https://wa.me/${digits}" target="_blank" rel="noopener noreferrer" title="WhatsApp · ${escapeHtml(formatPhone(client.phone))}" aria-label="Enviar WhatsApp">${appIcons.whatsapp}</a><a class="mvp-contact-btn call" href="tel:+${digits}" title="Llamar · ${escapeHtml(formatPhone(client.phone))}" aria-label="Llamar">${appIcons.phone}</a>${client.email ? `<a class="mvp-contact-btn mail" href="mailto:${escapeHtml(client.email)}" title="${escapeHtml(client.email)}" aria-label="Enviar email">${appIcons.mail}</a>` : `<span class="mvp-contact-btn mail" data-disabled title="Sin email cargado" aria-label="Sin email cargado">${appIcons.mail}</span>`}</div></div><div class="mvp-lead-actions"><button type="button" class="secondary mvp-icon-btn" data-edit-client="${client.id}" aria-controls="mvp-lead-form" title="Editar" aria-label="Editar ${escapeHtml(client.name)}">${appIcons.edit}</button><button type="button" class="delete mvp-icon-btn" data-delete="clients" data-id="${client.id}" title="Eliminar" aria-label="Eliminar ${escapeHtml(client.name)}">×</button></div></div>${matchesForLead(client)}</article>`;
}
...
export function renderMvpLeads(container: HTMLElement): void {
  ...
  const leads = leadRows();
  container.innerHTML = `<div class="mvp-page-heading"><div><h1>Leads</h1></div><button type="button" data-toggle="client-form">Nuevo lead</button></div><form id="mvp-lead-form" class="mvp-lead-form ${state.openForms.client ? '' : 'collapsed'}"><div class="mvp-form-heading"><h2>${editing ? `Editar ${escapeHtml(editing.name)}` : 'Nuevo lead'}</h2><button type="button" class="quiet-button" data-cancel-client-edit>Cerrar</button></div><label>Nombre<input name="name" value="${value(editing, 'name')}" required></label><label>Número de WhatsApp<input name="phone" value="${value(editing, 'phone')}" inputmode="tel" required></label><label>Email<input name="email" type="email" value="${value(editing, 'email')}" placeholder="cliente@correo.com"></label><label>Lugar o propiedad de interés<input name="interest" value="${value(editing, 'interest')}" placeholder="Ej. Departamento de 2 dormitorios en General Paz, apto crédito" required></label><label>Presupuesto<input name="budget" value="${value(editing, 'budget')}" placeholder="Ej. USD 85.000"></label><label>Temperatura<select name="temperature">${['Caliente', 'Tibio', 'Frío'].map((t) => `<option value="${t}"${(editing?.temperature ?? 'Tibio') === t ? ' selected' : ''}>${t}</option>`).join('')}</select></label><div data-lead-error class="form-error" hidden></div><button type="submit">${editing ? 'Guardar cambios' : 'Guardar lead'}</button></form><div class="mvp-lead-toolbar"><label><span>Buscar</span><input id="mvp-lead-search" type="search" value="${escapeHtml(searchText)}" placeholder="Nombre, WhatsApp, interés o presupuesto"></label><strong id="mvp-lead-count">${leads.length} leads</strong></div><div id="mvp-lead-results" class="mvp-lead-list">${leads.map(card).join('') || '<p class="empty-state">No hay leads para mostrar.</p>'}</div>`;
